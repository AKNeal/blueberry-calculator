// ============================================
// RECIPE REGISTRY — single source of truth
// Add a recipe here, it appears on the hub + gets its own page automatically
// ============================================

export interface Ingredient {
  amount: string;       // "2 cups" or "1/2 tsp"
  item: string;         // "blueberries, fresh or frozen"
  berries?: number;     // optional — # of median blueberries this represents
}

export interface Recipe {
  slug: string;
  title: string;
  short: string;        // listing-card title
  category: RecipeCategory;
  yield: string;        // "9-inch pie, serves 8"
  time: {
    prep: string;
    cook: string;
    total: string;
  };
  difficulty: "Easy" | "Medium" | "Advanced";
  // Image paths — relative to /public.
  // Convention: /placeholders/recipes/{slug}-hero.jpg until the real photo is added,
  // then move to /recipes/{slug}-hero.jpg and update both fields here.
  heroImage: string;
  ingredientsImage: string;
  ingredients: Ingredient[];
  steps: string[];
  // Which calculator on the site this pairs with
  relatedCalculator?: string;
  notes?: string[];
  // Marked true ONLY after Neal has personally tested the recipe in his own kitchen.
  // When true, the "TESTED" stamp renders on the recipe hub card and recipe page.
  tested?: boolean;
}

export type RecipeCategory =
  | "Pies & Tarts"
  | "Breakfast"
  | "Preserves"
  | "Baked Goods";

export const RECIPE_CATEGORIES: RecipeCategory[] = [
  "Pies & Tarts",
  "Breakfast",
  "Preserves",
  "Baked Goods",
];

// Image path helpers.
// All recipes currently use a single shared placeholder SVG — the slug+kind
// arguments are kept in the signature for self-documentation only.
//
// Workflow when a real photo arrives:
//   1. Drop the file at /public/recipes/{slug}-{hero|ingredients}.jpg
//   2. In that recipe's entry below, swap `placeholderImg("foo", "hero")`
//      for `recipeImg("foo", "hero")` and the same for ingredients.
//   3. Commit. The new image will appear on next build.
//
// See /public/placeholders/README.md for image specs (1200×800, JPEG, etc.).
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const placeholderImg = (_slug: string, _kind: "hero" | "ingredients") =>
  `/placeholders/recipe-placeholder.svg`;

// eslint-disable-next-line @typescript-eslint/no-unused-vars
const recipeImg = (slug: string, kind: "hero" | "ingredients") =>
  `/recipes/${slug}-${kind}.jpg`;

export const RECIPES: Recipe[] = [
  // ============ PIES & TARTS ============
  {
    slug: "classic-blueberry-pie",
    title: "Classic Blueberry Pie",
    short: "Classic Blueberry Pie",
    category: "Pies & Tarts",
    yield: "One 9-inch pie · serves 8",
    time: { prep: "25 min", cook: "55 min", total: "1 hr 20 min + cooling" },
    difficulty: "Medium",
    heroImage: recipeImg("classic-blueberry-pie", "hero"),
    ingredientsImage: recipeImg("classic-blueberry-pie", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 (9-inch)", item: "pie crusts, chilled (homemade or store-bought)" },
      { amount: "6 cups", item: "fresh blueberries", berries: 594 },
      { amount: "3/4 cup", item: "granulated sugar" },
      { amount: "1/4 cup", item: "cornstarch" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "1 tsp", item: "lemon zest" },
      { amount: "1/4 tsp", item: "ground cinnamon" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "2 tbsp", item: "unsalted butter, cut into small pieces" },
      { amount: "1", item: "egg, beaten (for wash)" },
      { amount: "1 tbsp", item: "coarse sugar (for topping)" },
    ],
    steps: [
      "Preheat oven to 400°F (205°C). Place a rack in the lower third and put a foil-lined baking sheet on it.",
      "Roll out one pie crust and fit it into a 9-inch pie dish. Refrigerate while making the filling.",
      "In a large bowl, toss blueberries with sugar, cornstarch, lemon juice, lemon zest, cinnamon, and salt until evenly coated.",
      "Pour the filling into the chilled crust and mound it slightly in the center. Dot the top with butter pieces.",
      "Roll out the second crust. Either place it whole over the pie and cut 4–5 steam vents, or cut into strips and weave a lattice. Crimp the edges.",
      "Brush the top crust with beaten egg and sprinkle with coarse sugar.",
      "Bake for 20 minutes at 400°F, then reduce heat to 375°F (190°C) and bake another 35–45 minutes, until the filling is bubbling thickly in the center and the crust is deep golden.",
      "Cool on a wire rack for at least 3 hours — this is not optional. The filling needs time to set or it will run.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "Frozen blueberries work — do not thaw, and add an extra tablespoon of cornstarch.",
      "If the crust edges brown too fast, cover them with foil strips or a pie shield.",
    ],
  },

  // ============ BREAKFAST ============
  {
    slug: "blueberry-muffins",
    title: "Bakery-Style Blueberry Muffins",
    short: "Blueberry Muffins",
    category: "Breakfast",
    yield: "12 muffins",
    time: { prep: "15 min", cook: "25 min", total: "40 min" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-muffins", "hero"),
    ingredientsImage: recipeImg("blueberry-muffins", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "all-purpose flour" },
      { amount: "1 cup", item: "granulated sugar" },
      { amount: "2 tsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1/2 cup", item: "unsalted butter, melted and slightly cooled" },
      { amount: "2", item: "large eggs" },
      { amount: "1/2 cup", item: "whole milk" },
      { amount: "1/2 cup", item: "sour cream or Greek yogurt" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1 1/2 cups", item: "fresh blueberries", berries: 148 },
      { amount: "2 tbsp", item: "coarse sugar (for topping)" },
    ],
    steps: [
      "Preheat oven to 425°F (220°C). Line a standard 12-cup muffin tin with paper liners.",
      "Whisk flour, sugar, baking powder, and salt in a large bowl.",
      "In a separate bowl, whisk melted butter, eggs, milk, sour cream, and vanilla until smooth.",
      "Pour the wet ingredients into the dry and fold with a spatula until just combined — lumps are fine. Do not overmix.",
      "Toss the blueberries with 1 tablespoon of flour (from the measured amount above), then fold gently into the batter.",
      "Divide the batter evenly among the 12 cups, filling them to the top. Sprinkle each with coarse sugar.",
      "Bake at 425°F for 5 minutes, then reduce heat to 375°F (190°C) without opening the oven. Bake for another 18–20 minutes, until tops are domed and a toothpick comes out clean.",
      "Let cool in the pan for 5 minutes, then transfer to a wire rack.",
    ],
    relatedCalculator: "muffin-scaler",
    notes: [
      "The initial 425°F burst gives you the tall, domed bakery shape.",
      "Tossing berries in flour keeps them suspended instead of sinking.",
    ],
  },

  {
    slug: "blueberry-pancakes",
    title: "Buttermilk Blueberry Pancakes",
    short: "Blueberry Pancakes",
    category: "Breakfast",
    yield: "About 12 pancakes · serves 4",
    time: { prep: "10 min", cook: "15 min", total: "25 min" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-pancakes", "hero"),
    ingredientsImage: recipeImg("blueberry-pancakes", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/2 cups", item: "all-purpose flour" },
      { amount: "2 tbsp", item: "granulated sugar" },
      { amount: "2 tsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "baking soda" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1 1/4 cups", item: "buttermilk" },
      { amount: "2", item: "large eggs" },
      { amount: "3 tbsp", item: "unsalted butter, melted (plus more for the pan)" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1 cup", item: "fresh blueberries", berries: 99 },
    ],
    steps: [
      "Whisk flour, sugar, baking powder, baking soda, and salt in a large bowl.",
      "In a separate bowl, whisk buttermilk, eggs, melted butter, and vanilla.",
      "Pour the wet into the dry and stir just until combined. The batter should be thick and lumpy — overmixing makes tough pancakes.",
      "Heat a non-stick skillet or griddle over medium-low. Butter it lightly.",
      "Pour 1/4 cup of batter per pancake. Immediately drop 6–8 blueberries onto each one.",
      "Cook 2–3 minutes until bubbles form across the surface and the edges look set. Flip and cook another 1–2 minutes until golden.",
      "Keep finished pancakes warm in a 200°F oven in a single layer while you cook the rest.",
    ],
    notes: [
      "Dropping berries onto the batter in the pan (rather than stirring them in) prevents the batter from turning gray.",
      "If you only have regular milk, add 1 tablespoon of lemon juice or vinegar and let it sit 5 minutes.",
    ],
  },

  // ============ PRESERVES ============
  {
    slug: "blueberry-jam",
    title: "Small-Batch Blueberry Jam",
    short: "Blueberry Jam",
    category: "Preserves",
    yield: "About 3 cups · three 8-oz jars",
    time: { prep: "10 min", cook: "25 min", total: "35 min" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-jam", "hero"),
    ingredientsImage: recipeImg("blueberry-jam", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 lb", item: "fresh blueberries (about 6 cups)", berries: 604 },
      { amount: "1 1/2 cups", item: "granulated sugar" },
      { amount: "3 tbsp", item: "lemon juice (about 1 large lemon)" },
      { amount: "1 tsp", item: "lemon zest" },
      { amount: "1/4 tsp", item: "fine salt" },
    ],
    steps: [
      "Combine blueberries, sugar, lemon juice, zest, and salt in a heavy wide-bottomed pot. Stir to coat.",
      "Let the mixture sit for 10 minutes — the sugar will draw out the blueberry juice.",
      "Place a small plate in the freezer (you'll use this to test the set later).",
      "Bring the mixture to a boil over medium-high heat, stirring often. Once boiling, mash about half the berries with a potato masher or the back of a spoon.",
      "Continue boiling, stirring frequently to prevent scorching, for about 15–20 minutes. The jam should thicken and reduce by roughly a third.",
      "Test the set: drop a small spoonful on the cold plate. Wait 30 seconds, then push it with your finger. If it wrinkles and holds its shape, it's done. If it runs, cook another 2–3 minutes and test again.",
      "Ladle into clean 8-oz jars, leaving 1/4 inch of headspace. Cool to room temperature, then refrigerate.",
    ],
    relatedCalculator: "jam-yield",
    notes: [
      "Refrigerator jam keeps for 3 weeks. For shelf-stable jam, process sealed jars in a boiling water bath for 10 minutes.",
      "No pectin needed — the lemon juice and cooking time do the work.",
    ],
  },

  // ============ BAKED GOODS ============
  {
    slug: "blueberry-scones",
    title: "Blueberry Cream Scones",
    short: "Blueberry Scones",
    category: "Baked Goods",
    yield: "8 scones",
    time: { prep: "15 min", cook: "22 min", total: "37 min" },
    difficulty: "Medium",
    heroImage: recipeImg("blueberry-scones", "hero"),
    ingredientsImage: recipeImg("blueberry-scones", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 1/2 cups", item: "all-purpose flour" },
      { amount: "1/3 cup", item: "granulated sugar" },
      { amount: "1 tbsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1/2 cup", item: "cold unsalted butter, cubed" },
      { amount: "1 cup", item: "fresh blueberries", berries: 99 },
      { amount: "3/4 cup", item: "heavy cream, plus 2 tbsp for brushing" },
      { amount: "1", item: "large egg" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "2 tbsp", item: "coarse sugar (for topping)" },
    ],
    steps: [
      "Preheat oven to 400°F (205°C). Line a baking sheet with parchment.",
      "Whisk flour, sugar, baking powder, and salt in a large bowl.",
      "Add cold cubed butter. Use your fingertips or a pastry cutter to work it in until the mixture looks like coarse crumbs with some pea-sized butter pieces remaining.",
      "Gently toss the blueberries into the flour mixture.",
      "Whisk the 3/4 cup cream, egg, and vanilla in a separate bowl. Pour into the flour and stir with a fork until a shaggy dough forms.",
      "Turn the dough onto a lightly floured surface. Pat — don't roll — into an 8-inch disc about 1 inch thick. Cut into 8 wedges with a sharp knife.",
      "Transfer wedges to the baking sheet, spacing them 2 inches apart. Brush tops with the extra cream and sprinkle with coarse sugar.",
      "Bake for 20–22 minutes, until the tops are golden and the edges look set. Cool for 5 minutes before serving.",
    ],
    notes: [
      "Keep everything cold. Warm butter = flat scones.",
      "Frozen blueberries work — use straight from the freezer, do not thaw.",
    ],
  },

  {
    slug: "blueberry-cobbler",
    title: "Southern Blueberry Cobbler",
    short: "Blueberry Cobbler",
    category: "Baked Goods",
    yield: "9×9 pan · serves 8",
    time: { prep: "15 min", cook: "45 min", total: "1 hr" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-cobbler", "hero"),
    ingredientsImage: placeholderImg("blueberry-cobbler", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "6 cups", item: "fresh blueberries", berries: 594 },
      { amount: "3/4 cup", item: "granulated sugar (for filling)" },
      { amount: "2 tbsp", item: "cornstarch" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "1 cup", item: "all-purpose flour" },
      { amount: "1/2 cup", item: "granulated sugar (for topping)" },
      { amount: "2 tsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "6 tbsp", item: "cold unsalted butter, cubed" },
      { amount: "1/2 cup", item: "boiling water" },
      { amount: "1 tbsp", item: "coarse sugar (for topping)" },
    ],
    steps: [
      "Preheat oven to 375°F (190°C). Butter a 9×9-inch baking dish.",
      "In a bowl, toss blueberries with 3/4 cup sugar, cornstarch, and lemon juice. Pour into the prepared dish and spread evenly.",
      "In another bowl, whisk flour, 1/2 cup sugar, baking powder, and salt.",
      "Add cold butter cubes and work them into the flour with your fingertips until the mixture resembles coarse sand.",
      "Pour the boiling water over the flour mixture and stir just until it comes together into a thick, spoonable batter.",
      "Drop spoonfuls of the batter over the blueberries — don't worry about covering every gap, the topping will spread as it bakes.",
      "Sprinkle coarse sugar over the top.",
      "Bake for 40–45 minutes, until the filling bubbles thickly around the edges and the topping is deep golden brown.",
      "Cool for at least 20 minutes before serving. Excellent with vanilla ice cream.",
    ],
    notes: [
      "The boiling water is the trick — it activates the baking powder instantly and gives the topping a cakey-biscuit texture.",
    ],
  },

  // ============ PIES & TARTS (batch 2) ============
  {
    slug: "blueberry-galette",
    title: "Rustic Blueberry Galette",
    short: "Blueberry Galette",
    category: "Pies & Tarts",
    yield: "One 10-inch galette · serves 6",
    time: { prep: "20 min", cook: "40 min", total: "1 hr + cooling" },
    difficulty: "Medium",
    heroImage: recipeImg("blueberry-galette", "hero"),
    ingredientsImage: recipeImg("blueberry-galette", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 (9-inch)", item: "pie crust, chilled (homemade or store-bought)" },
      { amount: "3 cups", item: "fresh blueberries", berries: 297 },
      { amount: "1/3 cup", item: "granulated sugar" },
      { amount: "3 tbsp", item: "cornstarch" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "1/2 tsp", item: "lemon zest" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "1 tbsp", item: "unsalted butter, cut into small pieces" },
      { amount: "1", item: "egg, beaten (for wash)" },
      { amount: "1 tbsp", item: "coarse sugar (for topping)" },
    ],
    steps: [
      "Preheat oven to 400°F (205°C). Line a baking sheet with parchment.",
      "Roll the crust into a rough 12-inch circle on the parchment. Ragged edges are fine — that's the point of a galette.",
      "Toss blueberries with sugar, cornstarch, lemon juice, zest, and salt.",
      "Mound the filling in the center of the crust, leaving a 2-inch border. Dot with butter.",
      "Fold the border up and over the filling, pleating every 2–3 inches. The center stays open.",
      "Brush the crust with beaten egg and sprinkle with coarse sugar.",
      "Bake 35–40 minutes, until the crust is deep golden and the filling bubbles in the center.",
      "Cool on the sheet for at least 45 minutes before slicing — the filling sets as it cools.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "If juice leaks through a pleat, leave it — it bakes into a jammy edge.",
      "Frozen berries work; do not thaw, and add 1 extra teaspoon of cornstarch.",
    ],
  },

  {
    slug: "blueberry-hand-pies",
    title: "Baked Blueberry Hand Pies",
    short: "Blueberry Hand Pies",
    category: "Pies & Tarts",
    yield: "8 hand pies",
    time: { prep: "30 min", cook: "22 min", total: "1 hr" },
    difficulty: "Medium",
    heroImage: recipeImg("blueberry-hand-pies", "hero"),
    ingredientsImage: recipeImg("blueberry-hand-pies", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 (9-inch)", item: "pie crusts, chilled" },
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "1/4 cup", item: "granulated sugar" },
      { amount: "2 tbsp", item: "cornstarch" },
      { amount: "2 tsp", item: "lemon juice" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "1", item: "egg, beaten (for wash)" },
      { amount: "1 tbsp", item: "coarse sugar (for topping)" },
    ],
    steps: [
      "Cook blueberries, sugar, cornstarch, lemon juice, and salt in a saucepan over medium heat for 5–6 minutes, until thick and jammy. Cool completely — hot filling melts the crust.",
      "Preheat oven to 400°F (205°C). Line a baking sheet with parchment.",
      "Roll each crust to a 12-inch circle and cut eight 4-inch rounds total, re-rolling scraps once.",
      "Spoon 1 heaping tablespoon of cooled filling onto one half of each round, leaving a 1/2-inch border.",
      "Brush the border with egg, fold into a half-moon, and press the edge closed with a fork.",
      "Space the pies on the sheet, brush tops with egg, sprinkle with coarse sugar, and cut a small vent slit in each.",
      "Bake 20–22 minutes, until deep golden. Cool 15 minutes — the filling is lava-hot straight out.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "Overfilling is the #1 cause of blowouts. One heaping tablespoon, no more.",
      "Unbaked pies freeze well; bake from frozen and add 3–4 minutes.",
    ],
  },

  {
    slug: "lemon-blueberry-tart",
    title: "Lemon Blueberry Cream Tart",
    short: "Lemon Blueberry Tart",
    category: "Pies & Tarts",
    yield: "One 9-inch tart · serves 8",
    time: { prep: "25 min", cook: "22 min", total: "3 hr (incl. chilling)" },
    difficulty: "Advanced",
    heroImage: recipeImg("lemon-blueberry-tart", "hero"),
    ingredientsImage: recipeImg("lemon-blueberry-tart", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/4 cups", item: "all-purpose flour" },
      { amount: "1/4 cup", item: "powdered sugar" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "1/2 cup", item: "cold unsalted butter, cubed" },
      { amount: "1", item: "large egg yolk" },
      { amount: "8 oz", item: "cream cheese, softened" },
      { amount: "1/3 cup", item: "powdered sugar (for filling)" },
      { amount: "1/2 cup", item: "heavy cream, cold" },
      { amount: "2 tbsp", item: "lemon juice" },
      { amount: "1 tbsp", item: "lemon zest (about 2 lemons)" },
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "2 tbsp", item: "blueberry jam or apricot jam (for glaze)" },
    ],
    steps: [
      "Pulse flour, powdered sugar, and salt in a food processor. Add butter and pulse to coarse crumbs, then add the yolk and pulse until the dough clumps.",
      "Press the dough evenly into a 9-inch tart pan with a removable bottom, going up the sides. Freeze 15 minutes.",
      "Preheat oven to 375°F (190°C). Prick the base with a fork and bake 20–22 minutes, until golden. Cool completely.",
      "Beat cream cheese with 1/3 cup powdered sugar until smooth. Add lemon juice and zest.",
      "In a separate bowl, whip the cold cream to stiff peaks, then fold into the cream cheese mixture.",
      "Spread the filling into the cooled shell and arrange blueberries over the top in a single tight layer.",
      "Warm the jam with 1 teaspoon of water and brush over the berries for shine.",
      "Chill at least 2 hours before removing the ring and slicing.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "The shell must be fully cool before filling or the cream will slide.",
      "Skip the glaze if serving within the hour; it mainly protects the berries overnight.",
    ],
  },

  // ============ BREAKFAST (batch 2) ============
  {
    slug: "blueberry-baked-oatmeal",
    title: "Blueberry Baked Oatmeal",
    short: "Baked Oatmeal",
    category: "Breakfast",
    yield: "9×9 pan · serves 6",
    time: { prep: "10 min", cook: "40 min", total: "50 min" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-baked-oatmeal", "hero"),
    ingredientsImage: recipeImg("blueberry-baked-oatmeal", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "old-fashioned rolled oats" },
      { amount: "1 tsp", item: "baking powder" },
      { amount: "1 tsp", item: "ground cinnamon" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1 3/4 cups", item: "milk (any kind)" },
      { amount: "1/4 cup", item: "maple syrup" },
      { amount: "1", item: "large egg" },
      { amount: "3 tbsp", item: "unsalted butter, melted" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "2 cups", item: "fresh blueberries, divided", berries: 198 },
    ],
    steps: [
      "Preheat oven to 375°F (190°C). Butter a 9×9-inch baking dish.",
      "Stir oats, baking powder, cinnamon, and salt in a large bowl.",
      "Whisk milk, maple syrup, egg, melted butter, and vanilla in a second bowl.",
      "Scatter 1 1/2 cups of the blueberries over the bottom of the dish. Cover with the oat mixture, then pour the liquid evenly over everything.",
      "Scatter the remaining 1/2 cup berries on top.",
      "Bake 35–40 minutes, until the top is golden and the center no longer jiggles.",
      "Rest 10 minutes before serving. Serve warm, with a splash of cold milk.",
    ],
    notes: [
      "Refrigerates well for 4 days; reheat squares in the microwave for 60 seconds.",
      "Use rolled oats only — quick oats turn to mush and steel-cut won't cook through.",
    ],
  },

  {
    slug: "blueberry-banana-smoothie",
    title: "Blueberry Banana Smoothie",
    short: "Blueberry Smoothie",
    category: "Breakfast",
    yield: "2 large smoothies · about 24 oz",
    time: { prep: "5 min", cook: "0 min", total: "5 min" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-banana-smoothie", "hero"),
    ingredientsImage: recipeImg("blueberry-banana-smoothie", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/2 cups", item: "frozen blueberries", berries: 148 },
      { amount: "1", item: "ripe banana" },
      { amount: "3/4 cup", item: "plain Greek yogurt" },
      { amount: "3/4 cup", item: "milk (any kind)" },
      { amount: "1 tbsp", item: "honey or maple syrup (optional)" },
      { amount: "1 tbsp", item: "peanut or almond butter (optional)" },
    ],
    steps: [
      "Add milk and yogurt to the blender first, then banana, then frozen blueberries on top.",
      "Blend on high for 45–60 seconds, until completely smooth. Scrape down once if needed.",
      "Taste. Add honey only if your berries are tart. Blend 5 more seconds and pour.",
    ],
    relatedCalculator: "nutrition",
    notes: [
      "Frozen berries are non-negotiable — they replace ice without watering it down.",
      "Too thick: add milk a splash at a time. Too thin: add 3–4 more frozen berries.",
    ],
  },

  {
    slug: "blueberry-french-toast-bake",
    title: "Overnight Blueberry French Toast Bake",
    short: "French Toast Bake",
    category: "Breakfast",
    yield: "9×13 pan · serves 8",
    time: { prep: "20 min", cook: "50 min", total: "1 hr 10 min + overnight" },
    difficulty: "Medium",
    heroImage: recipeImg("blueberry-french-toast-bake", "hero"),
    ingredientsImage: recipeImg("blueberry-french-toast-bake", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 (1-lb) loaf", item: "brioche or challah, cut into 1-inch cubes (slightly stale is best)" },
      { amount: "8", item: "large eggs" },
      { amount: "2 cups", item: "whole milk" },
      { amount: "1/2 cup", item: "heavy cream" },
      { amount: "1/2 cup", item: "light brown sugar, packed" },
      { amount: "1 tbsp", item: "vanilla extract" },
      { amount: "1 tsp", item: "ground cinnamon" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "1/3 cup", item: "all-purpose flour (for streusel)" },
      { amount: "1/3 cup", item: "light brown sugar (for streusel)" },
      { amount: "4 tbsp", item: "cold unsalted butter, cubed (for streusel)" },
    ],
    steps: [
      "Butter a 9×13-inch baking dish. Spread half the bread cubes in the dish, scatter 1 cup of blueberries over them, then layer the remaining bread and another 1 cup of berries.",
      "Whisk eggs, milk, cream, 1/2 cup brown sugar, vanilla, cinnamon, and salt. Pour evenly over the bread, pressing cubes down to soak.",
      "Cover and refrigerate at least 4 hours, ideally overnight.",
      "Preheat oven to 350°F (175°C). While it heats, rub the streusel flour, brown sugar, and cold butter together into coarse crumbs.",
      "Uncover the dish, scatter the streusel on top, and bake 45–50 minutes, until puffed, golden, and set in the center (a knife should come out without wet custard).",
      "Rest 10 minutes. Serve with maple syrup.",
    ],
    notes: [
      "Fresh soft bread turns soggy — dry the cubes in a 300°F oven for 10 minutes if your loaf is fresh.",
      "If the top browns before the center sets, tent loosely with foil.",
    ],
  },

  // ============ PRESERVES (batch 2) ============
  {
    slug: "blueberry-syrup",
    title: "Homemade Blueberry Syrup",
    short: "Blueberry Syrup",
    category: "Preserves",
    yield: "About 1 1/2 cups",
    time: { prep: "5 min", cook: "15 min", total: "20 min" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-syrup", "hero"),
    ingredientsImage: recipeImg("blueberry-syrup", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "1/2 cup", item: "granulated sugar" },
      { amount: "1/2 cup", item: "water" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "1 pinch", item: "fine salt" },
    ],
    steps: [
      "Combine blueberries, sugar, water, lemon juice, and salt in a saucepan over medium heat.",
      "Bring to a simmer and cook 10 minutes, mashing the berries against the side of the pan as they soften.",
      "For smooth syrup, strain through a fine-mesh sieve, pressing on the solids. For chunky pancake-house style, skip the strain.",
      "Return to the pan and simmer 3–5 more minutes, until it coats the back of a spoon. It thickens further as it cools.",
      "Cool and store in a jar in the refrigerator for up to 2 weeks.",
    ],
    relatedCalculator: "jam-yield",
    notes: [
      "Frozen berries work identically here — no adjustment needed.",
      "It will look thin while hot. Do not reduce past the spoon-coating stage or it turns to jelly in the fridge.",
    ],
  },

  {
    slug: "blueberry-compote",
    title: "5-Ingredient Blueberry Compote",
    short: "Blueberry Compote",
    category: "Preserves",
    yield: "About 1 1/2 cups",
    time: { prep: "5 min", cook: "10 min", total: "15 min" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-compote", "hero"),
    ingredientsImage: recipeImg("blueberry-compote", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "fresh blueberries, divided", berries: 198 },
      { amount: "3 tbsp", item: "granulated sugar" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "1 tbsp", item: "water" },
      { amount: "1 pinch", item: "fine salt" },
    ],
    steps: [
      "Combine 1 1/2 cups of the blueberries, sugar, lemon juice, water, and salt in a small saucepan over medium heat.",
      "Simmer 8–10 minutes, stirring occasionally, until the berries burst and the liquid is glossy and slightly thickened.",
      "Off heat, stir in the remaining 1/2 cup whole berries — they keep their shape and give the compote texture.",
      "Serve warm over pancakes, yogurt, or ice cream, or refrigerate up to 1 week.",
    ],
    relatedCalculator: "jam-yield",
    notes: [
      "The two-stage berry add is the difference between compote and jam. Don't skip it.",
      "Swap 1 teaspoon of the water for vanilla extract or add a cinnamon stick while simmering.",
    ],
  },

  {
    slug: "blueberry-chia-jam",
    title: "No-Cook-Down Blueberry Chia Jam",
    short: "Blueberry Chia Jam",
    category: "Preserves",
    yield: "About 1 1/4 cups · one 12-oz jar",
    time: { prep: "5 min", cook: "10 min", total: "45 min (incl. setting)" },
    difficulty: "Easy",
    heroImage: recipeImg("blueberry-chia-jam", "hero"),
    ingredientsImage: recipeImg("blueberry-chia-jam", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "2 tbsp", item: "maple syrup or honey" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "2 tbsp", item: "chia seeds" },
      { amount: "1/2 tsp", item: "vanilla extract (optional)" },
    ],
    steps: [
      "Cook blueberries, maple syrup, and lemon juice in a saucepan over medium heat for 8–10 minutes, mashing as they soften, until fully broken down.",
      "Remove from heat and stir in chia seeds and vanilla.",
      "Rest 30 minutes at room temperature, stirring twice — the chia gels the jam without pectin or long cooking.",
      "Transfer to a jar and refrigerate. It thickens further overnight. Keeps 2 weeks.",
    ],
    relatedCalculator: "jam-yield",
    notes: [
      "Uses a third of the sugar of traditional jam — expect a fruit-forward, less candied flavor.",
      "If it sets too firm, stir in warm water a teaspoon at a time.",
    ],
  },

  // ============ BAKED GOODS (batch 2) ============
  {
    slug: "lemon-blueberry-loaf",
    title: "Glazed Lemon Blueberry Loaf",
    short: "Lemon Blueberry Loaf",
    category: "Baked Goods",
    yield: "One 9×5 loaf · 10 slices",
    time: { prep: "15 min", cook: "60 min", total: "1 hr 15 min + cooling" },
    difficulty: "Easy",
    heroImage: placeholderImg("lemon-blueberry-loaf", "hero"),
    ingredientsImage: placeholderImg("lemon-blueberry-loaf", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/2 cups", item: "all-purpose flour, plus 1 tbsp for the berries" },
      { amount: "2 tsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1 cup", item: "granulated sugar" },
      { amount: "2 tbsp", item: "lemon zest (about 2 lemons)" },
      { amount: "2", item: "large eggs" },
      { amount: "1/2 cup", item: "plain yogurt or sour cream" },
      { amount: "1/2 cup", item: "neutral oil" },
      { amount: "3 tbsp", item: "lemon juice, divided" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1 1/2 cups", item: "fresh blueberries", berries: 148 },
      { amount: "1 cup", item: "powdered sugar (for glaze)" },
    ],
    steps: [
      "Preheat oven to 350°F (175°C). Grease a 9×5-inch loaf pan and line with a parchment sling.",
      "Whisk flour, baking powder, and salt in a bowl.",
      "In a large bowl, rub the lemon zest into the sugar with your fingers until fragrant. Whisk in eggs, yogurt, oil, 1 tablespoon of the lemon juice, and vanilla.",
      "Fold the dry ingredients into the wet until just combined.",
      "Toss blueberries with 1 tablespoon flour and fold in gently.",
      "Scrape into the pan and bake 55–60 minutes, until a skewer in the center comes out clean. Tent with foil at 40 minutes if browning fast.",
      "Cool 15 minutes in the pan, then lift out and cool completely on a rack.",
      "Whisk powdered sugar with the remaining 2 tablespoons lemon juice and drizzle over the fully cooled loaf.",
    ],
    relatedCalculator: "muffin-scaler",
    notes: [
      "Rubbing zest into sugar releases the oils — it's worth the 30 seconds.",
      "Glazing a warm loaf melts the glaze into a wet mess. Fully cool means fully cool.",
    ],
  },

  {
    slug: "blueberry-crumble-bars",
    title: "Blueberry Oat Crumble Bars",
    short: "Crumble Bars",
    category: "Baked Goods",
    yield: "8×8 pan · 12 bars",
    time: { prep: "15 min", cook: "45 min", total: "1 hr + cooling" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-crumble-bars", "hero"),
    ingredientsImage: placeholderImg("blueberry-crumble-bars", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/2 cups", item: "all-purpose flour" },
      { amount: "1 cup", item: "old-fashioned rolled oats" },
      { amount: "3/4 cup", item: "light brown sugar, packed" },
      { amount: "1/2 tsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "3/4 cup", item: "unsalted butter, melted" },
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "1/4 cup", item: "granulated sugar" },
      { amount: "1 tbsp", item: "cornstarch" },
      { amount: "1 tbsp", item: "lemon juice" },
    ],
    steps: [
      "Preheat oven to 375°F (190°C). Line an 8×8-inch pan with a parchment sling.",
      "Stir flour, oats, brown sugar, baking powder, and salt. Pour in melted butter and mix until evenly clumpy.",
      "Press two-thirds of the mixture firmly into the pan. Reserve the rest for topping.",
      "Toss blueberries with granulated sugar, cornstarch, and lemon juice; spread over the base.",
      "Crumble the reserved oat mixture over the berries in uneven clumps.",
      "Bake 40–45 minutes, until the top is golden and the filling bubbles at the edges.",
      "Cool completely in the pan — at least 2 hours — before lifting out and cutting. Warm bars fall apart.",
    ],
    notes: [
      "One bowl does the base and the topping. Resist adding extra butter; the bars firm up as they cool.",
      "Refrigerated bars cut cleaner and keep 5 days.",
    ],
  },

  {
    slug: "blueberry-crisp",
    title: "Old-Fashioned Blueberry Crisp",
    short: "Blueberry Crisp",
    category: "Baked Goods",
    yield: "9×9 pan · serves 8",
    time: { prep: "15 min", cook: "40 min", total: "55 min" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-crisp", "hero"),
    ingredientsImage: placeholderImg("blueberry-crisp", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "5 cups", item: "fresh blueberries", berries: 495 },
      { amount: "1/3 cup", item: "granulated sugar" },
      { amount: "2 tbsp", item: "cornstarch" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "3/4 cup", item: "all-purpose flour" },
      { amount: "3/4 cup", item: "old-fashioned rolled oats" },
      { amount: "2/3 cup", item: "light brown sugar, packed" },
      { amount: "1/2 tsp", item: "ground cinnamon" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "1/2 cup", item: "cold unsalted butter, cubed" },
    ],
    steps: [
      "Preheat oven to 375°F (190°C). Butter a 9×9-inch baking dish.",
      "Toss blueberries with granulated sugar, cornstarch, and lemon juice in the dish.",
      "Stir flour, oats, brown sugar, cinnamon, and salt in a bowl. Work in the cold butter with your fingertips until the mixture forms moist clumps.",
      "Scatter the topping evenly over the berries.",
      "Bake 35–40 minutes, until the topping is deep golden and the filling bubbles around the edges.",
      "Cool 15 minutes before serving. Vanilla ice cream is not optional in spirit.",
    ],
    notes: [
      "Crisp = oats in the topping, crumble = none. This is a crisp.",
      "Frozen berries: don't thaw, add 1 extra tablespoon of cornstarch, and expect 5 extra minutes.",
    ],
  },

  // ============ BATCH 3 — PIES & TARTS ============
  {
    slug: "blueberry-cream-cheese-pie",
    title: "No-Bake Blueberry Cream Cheese Pie",
    short: "Cream Cheese Pie",
    category: "Pies & Tarts",
    yield: "One 9-inch pie · serves 8",
    time: { prep: "20 min", cook: "10 min", total: "4 hr (incl. chilling)" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-cream-cheese-pie", "hero"),
    ingredientsImage: placeholderImg("blueberry-cream-cheese-pie", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 (9-inch)", item: "graham cracker crust, store-bought or homemade" },
      { amount: "8 oz", item: "cream cheese, softened" },
      { amount: "3/4 cup", item: "powdered sugar" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1 cup", item: "heavy cream, cold" },
      { amount: "3 cups", item: "fresh blueberries, divided", berries: 297 },
      { amount: "1/3 cup", item: "granulated sugar" },
      { amount: "2 tbsp", item: "cornstarch" },
      { amount: "2 tbsp", item: "water" },
      { amount: "1 tbsp", item: "lemon juice" },
    ],
    steps: [
      "Cook 1 1/2 cups of the blueberries with granulated sugar, cornstarch, water, and lemon juice in a saucepan over medium heat for 6–8 minutes, until thick and glossy. Cool completely.",
      "Beat cream cheese with powdered sugar and vanilla until completely smooth, scraping the bowl twice.",
      "In a separate cold bowl, whip the heavy cream to stiff peaks.",
      "Fold the whipped cream into the cream cheese mixture in three additions, keeping it light.",
      "Spread the filling into the crust and smooth the top.",
      "Stir the remaining 1 1/2 cups fresh blueberries into the cooled sauce, then spoon evenly over the filling.",
      "Chill at least 4 hours, preferably overnight, before slicing.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "Room-temperature cream cheese is the whole game — cold cheese gives you lumps you cannot beat out.",
      "The sauce must be fully cool before it touches the filling or the cream will weep.",
    ],
  },

  {
    slug: "blueberry-slab-pie",
    title: "Blueberry Slab Pie",
    short: "Blueberry Slab Pie",
    category: "Pies & Tarts",
    yield: "10×15 sheet pan · serves 20",
    time: { prep: "30 min", cook: "45 min", total: "1 hr 15 min + cooling" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-slab-pie", "hero"),
    ingredientsImage: placeholderImg("blueberry-slab-pie", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "4 (9-inch)", item: "pie crusts, chilled (or a double batch of dough)" },
      { amount: "10 cups", item: "fresh blueberries", berries: 990 },
      { amount: "1 1/4 cups", item: "granulated sugar" },
      { amount: "1/2 cup", item: "cornstarch" },
      { amount: "2 tbsp", item: "lemon juice" },
      { amount: "1 tbsp", item: "lemon zest" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1", item: "egg, beaten (for wash)" },
      { amount: "2 tbsp", item: "coarse sugar (for topping)" },
    ],
    steps: [
      "Preheat oven to 375°F (190°C).",
      "Roll two of the crusts together into one 12×17-inch rectangle and fit it into a 10×15-inch rimmed sheet pan, letting the dough come up the sides. Refrigerate.",
      "Toss blueberries with sugar, cornstarch, lemon juice, zest, and salt until evenly coated.",
      "Spread the filling across the chilled base in an even layer.",
      "Roll the remaining two crusts into a second 12×17-inch rectangle. Lay it over the filling, or cut it into 1-inch strips and lattice the top. Seal and crimp the edges.",
      "Brush with beaten egg, sprinkle with coarse sugar, and cut 8–10 steam vents if using a solid top.",
      "Bake 40–45 minutes, until the crust is deep golden and the filling bubbles at the corners.",
      "Cool at least 2 hours before cutting into squares. Slab pie is meant to be eaten out of hand.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "This is the recipe for a crowd — it cuts into 20 squares and travels better than a round pie.",
      "Cornstarch scales with volume, not intuition. Half a cup looks like too much; it is not.",
    ],
  },

  {
    slug: "blueberry-crumb-pie",
    title: "Blueberry Crumb Pie",
    short: "Blueberry Crumb Pie",
    category: "Pies & Tarts",
    yield: "One 9-inch pie · serves 8",
    time: { prep: "20 min", cook: "55 min", total: "1 hr 15 min + cooling" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-crumb-pie", "hero"),
    ingredientsImage: placeholderImg("blueberry-crumb-pie", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 (9-inch)", item: "pie crust, chilled" },
      { amount: "6 cups", item: "fresh blueberries", berries: 594 },
      { amount: "2/3 cup", item: "granulated sugar" },
      { amount: "1/4 cup", item: "cornstarch" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "1 cup", item: "all-purpose flour (for crumb)" },
      { amount: "1/2 cup", item: "light brown sugar, packed (for crumb)" },
      { amount: "1/2 tsp", item: "ground cinnamon (for crumb)" },
      { amount: "1/2 cup", item: "cold unsalted butter, cubed (for crumb)" },
    ],
    steps: [
      "Preheat oven to 400°F (205°C). Fit the crust into a 9-inch pie dish and refrigerate.",
      "Toss blueberries with granulated sugar, cornstarch, lemon juice, and salt. Pour into the chilled crust.",
      "Rub flour, brown sugar, cinnamon, and cold butter together with your fingertips until the mixture forms large moist clumps — stop while the clumps are still chunky.",
      "Scatter the crumb evenly over the filling, all the way to the edges.",
      "Bake 20 minutes at 400°F, then reduce to 375°F (190°C) and bake 30–35 minutes more, until the crumb is golden and the filling bubbles through.",
      "Cool at least 3 hours before slicing so the filling can set.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "Easier than a double-crust pie and more forgiving — no lattice, no crimping the top.",
      "If the crumb browns before the filling bubbles, tent loosely with foil.",
    ],
  },

  // ============ BATCH 3 — BREAKFAST ============
  {
    slug: "blueberry-waffles",
    title: "Crispy Blueberry Waffles",
    short: "Blueberry Waffles",
    category: "Breakfast",
    yield: "6 Belgian waffles",
    time: { prep: "10 min", cook: "20 min", total: "30 min" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-waffles", "hero"),
    ingredientsImage: placeholderImg("blueberry-waffles", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "all-purpose flour" },
      { amount: "1/4 cup", item: "cornstarch" },
      { amount: "2 tbsp", item: "granulated sugar" },
      { amount: "1 tbsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "baking soda" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1 3/4 cups", item: "buttermilk" },
      { amount: "2", item: "large eggs, separated" },
      { amount: "1/2 cup", item: "unsalted butter, melted" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1 1/2 cups", item: "fresh blueberries", berries: 148 },
    ],
    steps: [
      "Preheat the waffle iron on its highest setting. Heat the oven to 200°F with a wire rack inside.",
      "Whisk flour, cornstarch, sugar, baking powder, baking soda, and salt in a large bowl.",
      "Whisk buttermilk, egg yolks, melted butter, and vanilla in a second bowl.",
      "In a third clean bowl, beat the egg whites to soft peaks.",
      "Stir the wet ingredients into the dry until just combined, then fold in the egg whites in two additions.",
      "Ladle batter into the iron, scatter a handful of blueberries over the surface, and close. Cook 4–5 minutes, until deeply golden and steam has stopped.",
      "Move finished waffles directly to the oven rack — never stack them, or they steam themselves soggy.",
    ],
    notes: [
      "Cornstarch plus whipped whites is what makes these shatter-crisp instead of bready.",
      "Scattering berries into the iron rather than the batter keeps the waffle from turning purple and sticking.",
    ],
  },

  {
    slug: "blueberry-overnight-oats",
    title: "Blueberry Overnight Oats",
    short: "Overnight Oats",
    category: "Breakfast",
    yield: "2 jars · serves 2",
    time: { prep: "5 min", cook: "0 min", total: "5 min + overnight" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-overnight-oats", "hero"),
    ingredientsImage: placeholderImg("blueberry-overnight-oats", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 cup", item: "old-fashioned rolled oats" },
      { amount: "1 cup", item: "milk (any kind)" },
      { amount: "1/2 cup", item: "plain Greek yogurt" },
      { amount: "2 tbsp", item: "maple syrup" },
      { amount: "1 tbsp", item: "chia seeds" },
      { amount: "1/2 tsp", item: "vanilla extract" },
      { amount: "1 pinch", item: "fine salt" },
      { amount: "1 1/2 cups", item: "fresh blueberries, divided", berries: 148 },
    ],
    steps: [
      "Stir oats, milk, yogurt, maple syrup, chia seeds, vanilla, and salt together in a bowl until fully combined.",
      "Fold in 1 cup of the blueberries.",
      "Divide between two jars, seal, and refrigerate at least 6 hours or overnight.",
      "Top with the remaining 1/2 cup fresh blueberries just before eating.",
    ],
    relatedCalculator: "nutrition",
    notes: [
      "Keeps 4 days refrigerated, so a double batch covers most of a work week.",
      "Too thick in the morning? Loosen with a splash of milk and stir.",
    ],
  },

  {
    slug: "blueberry-granola",
    title: "Maple Blueberry Granola",
    short: "Blueberry Granola",
    category: "Breakfast",
    yield: "About 6 cups",
    time: { prep: "10 min", cook: "40 min", total: "1 hr (incl. cooling)" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-granola", "hero"),
    ingredientsImage: placeholderImg("blueberry-granola", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "4 cups", item: "old-fashioned rolled oats" },
      { amount: "1 cup", item: "sliced almonds or chopped pecans" },
      { amount: "1/2 cup", item: "unsweetened shredded coconut" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1 tsp", item: "ground cinnamon" },
      { amount: "1/2 cup", item: "maple syrup" },
      { amount: "1/3 cup", item: "neutral oil or melted coconut oil" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1 cup", item: "dried blueberries" },
    ],
    steps: [
      "Preheat oven to 300°F (150°C). Line a rimmed baking sheet with parchment.",
      "Toss oats, nuts, coconut, salt, and cinnamon in a large bowl.",
      "Pour in maple syrup, oil, and vanilla. Stir until every oat looks coated and slightly glossy.",
      "Spread onto the sheet and press the mixture down firmly with a spatula — compacting it is what creates clusters.",
      "Bake 35–40 minutes, rotating the pan once at the halfway point. Do not stir.",
      "Cool completely on the pan without touching it, at least 30 minutes. It crisps as it cools.",
      "Break into clusters and toss with the dried blueberries. Store airtight for 3 weeks.",
    ],
    notes: [
      "Dried berries go in after baking. Fresh or dried berries baked into granola turn into hard black specks.",
      "Stirring during baking is the #1 cluster killer. Leave it alone.",
    ],
  },

  // ============ BATCH 3 — PRESERVES ============
  {
    slug: "blueberry-butter",
    title: "Slow-Cooked Blueberry Butter",
    short: "Blueberry Butter",
    category: "Preserves",
    yield: "About 2 cups · two 8-oz jars",
    time: { prep: "10 min", cook: "1 hr 30 min", total: "1 hr 40 min" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-butter", "hero"),
    ingredientsImage: placeholderImg("blueberry-butter", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "6 cups", item: "fresh blueberries", berries: 594 },
      { amount: "3/4 cup", item: "granulated sugar" },
      { amount: "2 tbsp", item: "lemon juice" },
      { amount: "1/2 tsp", item: "ground cinnamon" },
      { amount: "1/4 tsp", item: "ground nutmeg" },
      { amount: "1/4 tsp", item: "fine salt" },
    ],
    steps: [
      "Combine blueberries, sugar, lemon juice, cinnamon, nutmeg, and salt in a heavy pot over medium heat.",
      "Bring to a simmer, then reduce heat to low and cook 60–90 minutes, stirring every 10 minutes, until the mixture is very thick and dark.",
      "Purée with an immersion blender until completely smooth — this is what makes it a butter rather than a jam.",
      "Continue cooking uncovered another 10–15 minutes if it still slumps on a spoon. It should hold a mound.",
      "Ladle into clean jars, cool, and refrigerate up to 3 weeks.",
    ],
    relatedCalculator: "jam-yield",
    notes: [
      "No pectin and no added liquid — all the thickening comes from long, slow evaporation.",
      "Low heat and frequent stirring near the end; scorched fruit butter is unsalvageable.",
    ],
  },

  {
    slug: "blueberry-bbq-sauce",
    title: "Blueberry BBQ Sauce",
    short: "Blueberry BBQ Sauce",
    category: "Preserves",
    yield: "About 2 cups",
    time: { prep: "10 min", cook: "30 min", total: "40 min" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-bbq-sauce", "hero"),
    ingredientsImage: placeholderImg("blueberry-bbq-sauce", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "3 cups", item: "fresh blueberries", berries: 297 },
      { amount: "1/2 cup", item: "ketchup" },
      { amount: "1/3 cup", item: "light brown sugar, packed" },
      { amount: "1/4 cup", item: "apple cider vinegar" },
      { amount: "2 tbsp", item: "Worcestershire sauce" },
      { amount: "1 tbsp", item: "Dijon mustard" },
      { amount: "2 tsp", item: "smoked paprika" },
      { amount: "1 tsp", item: "garlic powder" },
      { amount: "1/2 tsp", item: "onion powder" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1/4 tsp", item: "cayenne pepper (optional)" },
    ],
    steps: [
      "Combine every ingredient in a saucepan over medium heat.",
      "Bring to a simmer, then reduce heat to low and cook 25–30 minutes, stirring occasionally, until the berries have collapsed and the sauce has darkened and thickened.",
      "Purée with an immersion blender until smooth, or leave it rustic if you prefer texture.",
      "Taste and adjust: more vinegar for brightness, more brown sugar for sweetness, more cayenne for heat.",
      "Cool and refrigerate up to 2 weeks. Excellent on pork, chicken, and burgers.",
    ],
    relatedCalculator: "jam-yield",
    notes: [
      "The blueberries replace most of the sugar you would find in a bottled sauce — expect a sharper, fruitier finish.",
      "It thickens noticeably in the fridge. Pull it off the heat looser than you think you want.",
    ],
  },

  {
    slug: "pickled-blueberries",
    title: "Quick Pickled Blueberries",
    short: "Pickled Blueberries",
    category: "Preserves",
    yield: "About 2 cups · one pint jar",
    time: { prep: "10 min", cook: "5 min", total: "15 min + 24 hr resting" },
    difficulty: "Easy",
    heroImage: placeholderImg("pickled-blueberries", "hero"),
    ingredientsImage: placeholderImg("pickled-blueberries", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "3/4 cup", item: "white wine vinegar" },
      { amount: "1/2 cup", item: "water" },
      { amount: "1/3 cup", item: "granulated sugar" },
      { amount: "1 tsp", item: "fine salt" },
      { amount: "1", item: "cinnamon stick" },
      { amount: "4", item: "whole black peppercorns" },
      { amount: "2", item: "whole cloves" },
      { amount: "1", item: "bay leaf" },
    ],
    steps: [
      "Pack the blueberries into a clean pint jar, leaving 1/2 inch of headspace.",
      "Combine vinegar, water, sugar, salt, cinnamon stick, peppercorns, cloves, and bay leaf in a saucepan.",
      "Bring to a boil, stirring until the sugar and salt dissolve, then remove from heat and let stand 5 minutes.",
      "Pour the hot brine over the berries, covering them completely. Include the whole spices.",
      "Cool to room temperature, seal, and refrigerate at least 24 hours before using.",
    ],
    notes: [
      "Sharp, sweet, and savory — these belong on a cheese board, in a salad, or beside roast pork, not on a pancake.",
      "Keeps 1 month refrigerated. The berries soften over time; they are best in the first two weeks.",
    ],
  },

  // ============ BATCH 3 — BAKED GOODS ============
  {
    slug: "blueberry-coffee-cake",
    title: "Blueberry Streusel Coffee Cake",
    short: "Coffee Cake",
    category: "Baked Goods",
    yield: "9×9 pan · serves 9",
    time: { prep: "20 min", cook: "50 min", total: "1 hr 10 min + cooling" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-coffee-cake", "hero"),
    ingredientsImage: placeholderImg("blueberry-coffee-cake", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "all-purpose flour, plus 1 tbsp for the berries" },
      { amount: "2 tsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1/2 cup", item: "unsalted butter, softened" },
      { amount: "1 cup", item: "granulated sugar" },
      { amount: "2", item: "large eggs" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "3/4 cup", item: "sour cream" },
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "3/4 cup", item: "all-purpose flour (for streusel)" },
      { amount: "1/2 cup", item: "light brown sugar, packed (for streusel)" },
      { amount: "1 tsp", item: "ground cinnamon (for streusel)" },
      { amount: "6 tbsp", item: "cold unsalted butter, cubed (for streusel)" },
    ],
    steps: [
      "Preheat oven to 350°F (175°C). Grease a 9×9-inch pan and line with a parchment sling.",
      "Make the streusel first: rub the 3/4 cup flour, brown sugar, cinnamon, and cold butter together into coarse clumps. Refrigerate while you mix the cake.",
      "Whisk the 2 cups flour, baking powder, and salt in a bowl.",
      "Beat the softened butter and granulated sugar 3 minutes, until pale and fluffy. Add eggs one at a time, then vanilla.",
      "Add the dry ingredients in three additions, alternating with the sour cream, mixing only until each addition disappears.",
      "Toss the blueberries with 1 tablespoon flour and fold in.",
      "Spread the batter in the pan — it is thick, so smooth it with an offset spatula — and cover with the chilled streusel.",
      "Bake 45–50 minutes, until a skewer in the center comes out clean. Cool 20 minutes before cutting.",
    ],
    notes: [
      "Chilling the streusel keeps it in distinct clumps instead of melting into a sandy layer.",
      "The batter should be thick. Do not thin it out; that is what suspends the berries.",
    ],
  },

  {
    slug: "blueberry-banana-bread",
    title: "Blueberry Banana Bread",
    short: "Banana Bread",
    category: "Baked Goods",
    yield: "One 9×5 loaf · 10 slices",
    time: { prep: "15 min", cook: "65 min", total: "1 hr 20 min + cooling" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-banana-bread", "hero"),
    ingredientsImage: placeholderImg("blueberry-banana-bread", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 3/4 cups", item: "all-purpose flour, plus 1 tbsp for the berries" },
      { amount: "1 tsp", item: "baking soda" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1/2 tsp", item: "ground cinnamon" },
      { amount: "3", item: "very ripe bananas, mashed (about 1 1/2 cups)" },
      { amount: "3/4 cup", item: "light brown sugar, packed" },
      { amount: "1/2 cup", item: "unsalted butter, melted" },
      { amount: "2", item: "large eggs" },
      { amount: "1/4 cup", item: "plain yogurt or sour cream" },
      { amount: "2 tsp", item: "vanilla extract" },
      { amount: "1 1/2 cups", item: "fresh blueberries", berries: 148 },
    ],
    steps: [
      "Preheat oven to 350°F (175°C). Grease a 9×5-inch loaf pan and line with a parchment sling.",
      "Whisk the 1 3/4 cups flour, baking soda, salt, and cinnamon in a bowl.",
      "In a large bowl, stir mashed bananas with brown sugar, melted butter, eggs, yogurt, and vanilla until smooth.",
      "Fold the dry ingredients into the wet until just barely combined — a few streaks of flour are fine.",
      "Toss the blueberries with 1 tablespoon flour and fold in with three or four strokes.",
      "Scrape into the pan and bake 60–65 minutes, until a skewer comes out clean. Tent with foil at 40 minutes if the top darkens early.",
      "Cool in the pan 15 minutes, then lift out and cool completely before slicing.",
    ],
    notes: [
      "The bananas must be genuinely overripe — heavily spotted or black. Yellow bananas give you bland bread.",
      "Slicing warm banana bread tears it. Wait for it to cool fully.",
    ],
  },

  {
    slug: "blueberry-cheesecake-bars",
    title: "Blueberry Cheesecake Bars",
    short: "Cheesecake Bars",
    category: "Baked Goods",
    yield: "9×9 pan · 16 bars",
    time: { prep: "25 min", cook: "45 min", total: "5 hr (incl. chilling)" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-cheesecake-bars", "hero"),
    ingredientsImage: placeholderImg("blueberry-cheesecake-bars", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/2 cups", item: "graham cracker crumbs" },
      { amount: "1/4 cup", item: "granulated sugar (for crust)" },
      { amount: "6 tbsp", item: "unsalted butter, melted" },
      { amount: "16 oz", item: "cream cheese, softened" },
      { amount: "2/3 cup", item: "granulated sugar (for filling)" },
      { amount: "2", item: "large eggs, room temperature" },
      { amount: "1/2 cup", item: "sour cream" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "2 tsp", item: "vanilla extract" },
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
      { amount: "2 tbsp", item: "granulated sugar (for swirl)" },
      { amount: "1 tsp", item: "cornstarch" },
    ],
    steps: [
      "Preheat oven to 325°F (165°C). Line a 9×9-inch pan with a parchment sling.",
      "Cook 1 1/2 cups of the blueberries with 2 tablespoons sugar and the cornstarch over medium heat for 5–6 minutes, until thick. Press through a sieve if you want a smooth swirl. Cool.",
      "Stir graham crumbs, 1/4 cup sugar, and melted butter together and press firmly into the pan. Bake 10 minutes, then cool 10 minutes.",
      "Beat cream cheese and 2/3 cup sugar until smooth. Add eggs one at a time on low speed, then sour cream, lemon juice, and vanilla. Do not whip air into it.",
      "Pour the filling over the crust. Scatter the remaining 1/2 cup whole blueberries over the top.",
      "Dot the cooled blueberry sauce over the surface and drag a knife through it two or three times for a marbled swirl.",
      "Bake 35–40 minutes, until the edges are set and the center still has a slight wobble.",
      "Cool to room temperature, then refrigerate at least 4 hours before lifting out and cutting.",
    ],
    notes: [
      "Overbeating after the eggs go in causes cracks. Low speed, stop as soon as it is smooth.",
      "Wipe the knife between cuts for clean edges.",
    ],
  },

  // ============ BATCH 4 — PIES & TARTS ============
  {
    slug: "blueberry-peach-pie",
    title: "Blueberry Peach Pie",
    short: "Blueberry Peach Pie",
    category: "Pies & Tarts",
    yield: "One 9-inch pie · serves 8",
    time: { prep: "30 min", cook: "60 min", total: "1 hr 30 min + cooling" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-peach-pie", "hero"),
    ingredientsImage: placeholderImg("blueberry-peach-pie", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 (9-inch)", item: "pie crusts, chilled" },
      { amount: "3 cups", item: "fresh blueberries", berries: 297 },
      { amount: "3 cups", item: "peaches, peeled and sliced (about 4 medium)" },
      { amount: "3/4 cup", item: "granulated sugar" },
      { amount: "1/3 cup", item: "cornstarch" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "1/2 tsp", item: "ground cinnamon" },
      { amount: "1/4 tsp", item: "ground nutmeg" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "2 tbsp", item: "unsalted butter, cut into small pieces" },
      { amount: "1", item: "egg, beaten (for wash)" },
      { amount: "1 tbsp", item: "coarse sugar (for topping)" },
    ],
    steps: [
      "Preheat oven to 400°F (205°C) with a foil-lined baking sheet on the lower rack.",
      "Fit one crust into a 9-inch pie dish and refrigerate.",
      "Toss blueberries and peaches with sugar, cornstarch, lemon juice, cinnamon, nutmeg, and salt. Let stand 10 minutes so the fruit releases juice and the cornstarch hydrates.",
      "Pour the filling into the chilled crust, mounding slightly. Dot with butter.",
      "Top with the second crust — solid with vents, or a lattice — and crimp the edges.",
      "Brush with egg and sprinkle with coarse sugar.",
      "Bake 20 minutes at 400°F, then reduce to 375°F (190°C) for another 40–45 minutes, until the filling bubbles thickly at the center.",
      "Cool at least 3 hours. Peaches release more liquid than blueberries alone, so this one genuinely needs the full set time.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "Peel peaches easily by scoring an X in the base and dunking in boiling water for 30 seconds.",
      "The extra cornstarch is not a typo — peaches are far juicier than blueberries.",
    ],
  },

  {
    slug: "blueberry-sour-cream-pie",
    title: "Blueberry Sour Cream Pie",
    short: "Sour Cream Pie",
    category: "Pies & Tarts",
    yield: "One 9-inch pie · serves 8",
    time: { prep: "20 min", cook: "50 min", total: "1 hr 10 min + chilling" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-sour-cream-pie", "hero"),
    ingredientsImage: placeholderImg("blueberry-sour-cream-pie", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 (9-inch)", item: "pie crust, chilled" },
      { amount: "1 1/2 cups", item: "sour cream" },
      { amount: "3/4 cup", item: "granulated sugar" },
      { amount: "1", item: "large egg" },
      { amount: "3 tbsp", item: "all-purpose flour" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "2 1/2 cups", item: "fresh blueberries", berries: 248 },
      { amount: "1/3 cup", item: "all-purpose flour (for topping)" },
      { amount: "1/3 cup", item: "light brown sugar, packed (for topping)" },
      { amount: "3 tbsp", item: "cold unsalted butter, cubed (for topping)" },
    ],
    steps: [
      "Preheat oven to 400°F (205°C). Fit the crust into a 9-inch dish and refrigerate.",
      "Whisk sour cream, granulated sugar, egg, 3 tablespoons flour, vanilla, and salt until completely smooth.",
      "Fold in the blueberries gently — the custard should stay pale, not purple.",
      "Pour into the chilled crust and bake 25 minutes at 400°F.",
      "While it bakes, rub the topping flour, brown sugar, and cold butter into coarse crumbs.",
      "Scatter the topping over the partially baked pie and return to the oven for 20–25 minutes, until the topping is golden and the custard is set with only a slight wobble at the center.",
      "Cool completely, then refrigerate at least 3 hours before slicing.",
    ],
    relatedCalculator: "pie-dish",
    notes: [
      "A custard pie, not a fruit pie — the filling should set like cheesecake, not like jam.",
      "Do not skip the chill. Warm sour cream custard slumps when cut.",
    ],
  },

  // ============ BATCH 4 — BREAKFAST ============
  {
    slug: "blueberry-yogurt-parfait",
    title: "Layered Blueberry Yogurt Parfait",
    short: "Yogurt Parfait",
    category: "Breakfast",
    yield: "2 parfaits",
    time: { prep: "10 min", cook: "0 min", total: "10 min" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-yogurt-parfait", "hero"),
    ingredientsImage: placeholderImg("blueberry-yogurt-parfait", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "plain Greek yogurt" },
      { amount: "2 tbsp", item: "honey or maple syrup" },
      { amount: "1/2 tsp", item: "vanilla extract" },
      { amount: "2 cups", item: "fresh blueberries, divided", berries: 198 },
      { amount: "1 cup", item: "granola" },
      { amount: "1 tbsp", item: "lemon juice" },
      { amount: "1 tbsp", item: "granulated sugar" },
    ],
    steps: [
      "Crush 1 cup of the blueberries with the lemon juice and sugar in a small bowl. Let stand 5 minutes until syrupy — this is a raw quick-macerate, no cooking needed.",
      "Stir the yogurt with honey and vanilla.",
      "In two glasses, layer in this order: yogurt, granola, macerated berries. Repeat.",
      "Top with the remaining 1 cup of whole fresh blueberries and serve immediately.",
    ],
    relatedCalculator: "nutrition",
    notes: [
      "Assemble just before eating. Granola layered in advance turns to paste within an hour.",
      "For a make-ahead version, layer everything except the granola and add it at the table.",
    ],
  },

  {
    slug: "blueberry-breakfast-bars",
    title: "Soft Blueberry Breakfast Bars",
    short: "Breakfast Bars",
    category: "Breakfast",
    yield: "9×9 pan · 12 bars",
    time: { prep: "15 min", cook: "35 min", total: "50 min + cooling" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-breakfast-bars", "hero"),
    ingredientsImage: placeholderImg("blueberry-breakfast-bars", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "old-fashioned rolled oats" },
      { amount: "1 cup", item: "whole wheat flour" },
      { amount: "1/2 cup", item: "light brown sugar, packed" },
      { amount: "1 tsp", item: "baking powder" },
      { amount: "1 tsp", item: "ground cinnamon" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1", item: "large egg" },
      { amount: "1/2 cup", item: "milk (any kind)" },
      { amount: "1/3 cup", item: "melted coconut oil or unsalted butter" },
      { amount: "1/4 cup", item: "maple syrup" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "2 cups", item: "fresh blueberries", berries: 198 },
    ],
    steps: [
      "Preheat oven to 350°F (175°C). Line a 9×9-inch pan with a parchment sling.",
      "Stir oats, flour, brown sugar, baking powder, cinnamon, and salt in a large bowl.",
      "Whisk egg, milk, melted oil, maple syrup, and vanilla in a second bowl.",
      "Pour the wet into the dry and stir until evenly moistened. The mixture will be thick and scoopable, not pourable.",
      "Fold in the blueberries with a few strokes.",
      "Press into the pan in an even layer and smooth the top.",
      "Bake 32–35 minutes, until set and golden at the edges.",
      "Cool completely in the pan before lifting out and cutting into 12 bars.",
    ],
    relatedCalculator: "nutrition",
    notes: [
      "Lower in sugar than a muffin and sturdy enough for a lunchbox. Keeps 5 days.",
      "Wrap individually and freeze up to 3 months; thaw overnight.",
    ],
  },

  {
    slug: "blueberry-dutch-baby",
    title: "Blueberry Dutch Baby",
    short: "Dutch Baby",
    category: "Breakfast",
    yield: "One 10-inch skillet · serves 4",
    time: { prep: "10 min", cook: "22 min", total: "32 min" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-dutch-baby", "hero"),
    ingredientsImage: placeholderImg("blueberry-dutch-baby", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "3", item: "large eggs, room temperature" },
      { amount: "1/2 cup", item: "whole milk, room temperature" },
      { amount: "1/2 cup", item: "all-purpose flour" },
      { amount: "2 tbsp", item: "granulated sugar" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "3 tbsp", item: "unsalted butter" },
      { amount: "1 1/2 cups", item: "fresh blueberries", berries: 148 },
      { amount: "1 tbsp", item: "powdered sugar (for dusting)" },
      { amount: "1", item: "lemon, cut into wedges" },
    ],
    steps: [
      "Place a 10-inch cast iron skillet in the oven and preheat to 425°F (220°C). The skillet must be screaming hot.",
      "Blend eggs, milk, flour, sugar, vanilla, and salt until completely smooth and slightly frothy — about 30 seconds in a blender.",
      "Let the batter rest while the oven finishes heating. Room-temperature ingredients and a rested batter give the biggest rise.",
      "Carefully remove the hot skillet, add the butter, and swirl until melted and foaming.",
      "Pour the batter into the center and immediately scatter the blueberries over the top.",
      "Return to the oven and bake 20–22 minutes without opening the door. It will climb dramatically up the sides.",
      "Dust with powdered sugar and serve immediately with lemon wedges. It deflates within a minute — that is normal and expected.",
    ],
    notes: [
      "Opening the oven early collapses it. Watch through the window.",
      "Cold eggs and milk are the most common cause of a flat Dutch baby.",
    ],
  },

  // ============ BATCH 4 — PRESERVES ============
  {
    slug: "blueberry-lemon-curd",
    title: "Blueberry Lemon Curd",
    short: "Blueberry Curd",
    category: "Preserves",
    yield: "About 1 1/2 cups",
    time: { prep: "10 min", cook: "20 min", total: "30 min + chilling" },
    difficulty: "Advanced",
    heroImage: placeholderImg("blueberry-lemon-curd", "hero"),
    ingredientsImage: placeholderImg("blueberry-lemon-curd", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/2 cups", item: "fresh blueberries", berries: 148 },
      { amount: "1/3 cup", item: "lemon juice (about 2 lemons)" },
      { amount: "1 tbsp", item: "lemon zest" },
      { amount: "3/4 cup", item: "granulated sugar" },
      { amount: "3", item: "large eggs" },
      { amount: "2", item: "large egg yolks" },
      { amount: "1/4 tsp", item: "fine salt" },
      { amount: "6 tbsp", item: "cold unsalted butter, cubed" },
    ],
    steps: [
      "Simmer the blueberries with 2 tablespoons of the lemon juice for 5 minutes, until fully burst. Press through a fine-mesh sieve and discard the solids. You should have about 1/2 cup of purée.",
      "Whisk the purée, remaining lemon juice, zest, sugar, whole eggs, yolks, and salt in a heavy saucepan — off the heat — until completely smooth.",
      "Cook over medium-low heat, stirring constantly with a spatula and scraping the corners, for 8–12 minutes.",
      "The curd is done when it coats the back of a spoon and a finger drawn through leaves a clean line, around 170°F. Do not let it boil or the eggs will scramble.",
      "Remove from heat and whisk in the cold butter a few cubes at a time until glossy.",
      "Strain once more for insurance, press plastic wrap directly onto the surface, and chill at least 4 hours. It thickens substantially as it cools.",
    ],
    relatedCalculator: "jam-yield",
    notes: [
      "Constant stirring over gentle heat is the entire technique. Walk away and you get sweet scrambled eggs.",
      "If it does curdle slightly, an immersion blender will usually rescue it.",
    ],
  },

  {
    slug: "blueberry-simple-syrup",
    title: "Blueberry Simple Syrup for Drinks",
    short: "Blueberry Drink Syrup",
    category: "Preserves",
    yield: "About 1 1/4 cups",
    time: { prep: "5 min", cook: "12 min", total: "45 min (incl. steeping)" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-simple-syrup", "hero"),
    ingredientsImage: placeholderImg("blueberry-simple-syrup", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/2 cups", item: "fresh blueberries", berries: 148 },
      { amount: "1 cup", item: "granulated sugar" },
      { amount: "1 cup", item: "water" },
      { amount: "1 tsp", item: "lemon juice" },
      { amount: "2", item: "strips lemon peel (optional)" },
    ],
    steps: [
      "Combine sugar and water in a saucepan and heat, stirring, until the sugar fully dissolves.",
      "Add blueberries, lemon juice, and lemon peel if using. Bring to a gentle simmer.",
      "Simmer 10–12 minutes, pressing the berries against the side of the pan a few times to release colour and flavour.",
      "Remove from heat and steep 30 minutes off the burner — this deepens the flavour far more than longer boiling would.",
      "Strain through a fine-mesh sieve without pressing hard on the solids; pressing makes the syrup cloudy.",
      "Bottle and refrigerate up to 1 month.",
    ],
    notes: [
      "Thinner than pancake syrup by design — it needs to pour and mix into cold liquid.",
      "Use in lemonade, iced tea, soda water, or cocktails. About 1 tablespoon per glass.",
    ],
  },

  // ============ BATCH 4 — BAKED GOODS ============
  {
    slug: "blueberry-buckle",
    title: "Blueberry Buckle",
    short: "Blueberry Buckle",
    category: "Baked Goods",
    yield: "9×9 pan · serves 9",
    time: { prep: "20 min", cook: "50 min", total: "1 hr 10 min" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-buckle", "hero"),
    ingredientsImage: placeholderImg("blueberry-buckle", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 cups", item: "all-purpose flour" },
      { amount: "2 1/2 tsp", item: "baking powder" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "1/2 cup", item: "unsalted butter, softened" },
      { amount: "3/4 cup", item: "granulated sugar" },
      { amount: "1", item: "large egg" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1/2 cup", item: "whole milk" },
      { amount: "3 cups", item: "fresh blueberries", berries: 297 },
      { amount: "1/2 cup", item: "granulated sugar (for streusel)" },
      { amount: "1/3 cup", item: "all-purpose flour (for streusel)" },
      { amount: "1/2 tsp", item: "ground cinnamon (for streusel)" },
      { amount: "4 tbsp", item: "cold unsalted butter, cubed (for streusel)" },
    ],
    steps: [
      "Preheat oven to 375°F (190°C). Butter a 9×9-inch pan.",
      "Whisk the 2 cups flour, baking powder, and salt.",
      "Cream the softened butter with 3/4 cup sugar until light, about 3 minutes. Beat in the egg and vanilla.",
      "Add the dry ingredients alternately with the milk, mixing just until combined. The batter will be very stiff.",
      "Spread the batter in the pan — an offset spatula helps — then scatter the blueberries evenly over the top. Do not fold them in.",
      "Rub the streusel ingredients together into coarse crumbs and scatter over the berries.",
      "Bake 45–50 minutes, until a skewer inserted into the cake portion comes out clean.",
      "Cool 20 minutes before cutting. The cake rises up around the fruit as it bakes — that buckling is where the name comes from.",
    ],
    notes: [
      "A buckle differs from a coffee cake in that the fruit sits on top and sinks in during baking.",
      "The batter is meant to be stiff. If it spreads easily, you have overmixed.",
    ],
  },

  {
    slug: "blueberry-cookies",
    title: "Soft Blueberry White Chocolate Cookies",
    short: "Blueberry Cookies",
    category: "Baked Goods",
    yield: "24 cookies",
    time: { prep: "20 min", cook: "13 min", total: "1 hr (incl. chilling)" },
    difficulty: "Medium",
    heroImage: placeholderImg("blueberry-cookies", "hero"),
    ingredientsImage: placeholderImg("blueberry-cookies", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "2 1/4 cups", item: "all-purpose flour" },
      { amount: "1/2 tsp", item: "baking soda" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "3/4 cup", item: "unsalted butter, softened" },
      { amount: "3/4 cup", item: "light brown sugar, packed" },
      { amount: "1/4 cup", item: "granulated sugar" },
      { amount: "1", item: "large egg" },
      { amount: "1", item: "large egg yolk" },
      { amount: "2 tsp", item: "vanilla extract" },
      { amount: "1 tsp", item: "lemon zest" },
      { amount: "1 cup", item: "dried blueberries" },
      { amount: "3/4 cup", item: "white chocolate chips" },
    ],
    steps: [
      "Whisk flour, baking soda, and salt in a bowl.",
      "Cream butter with both sugars for 3 minutes until pale and fluffy. Beat in the egg, yolk, vanilla, and lemon zest.",
      "Fold in the dry ingredients until just combined, then stir in the dried blueberries and white chocolate.",
      "Chill the dough at least 30 minutes — this prevents spreading and deepens flavour.",
      "Preheat oven to 350°F (175°C) and line two sheets with parchment.",
      "Scoop 2-tablespoon balls, spacing them 2 inches apart. Do not flatten.",
      "Bake 11–13 minutes, until the edges are set but the centers still look slightly underdone.",
      "Cool on the sheet 5 minutes before transferring. They finish setting as they cool, which is what keeps them soft.",
    ],
    notes: [
      "Dried blueberries, not fresh. Fresh berries release water and turn cookie dough into cake batter.",
      "Pulling them out while the centers look underbaked is the difference between soft and crunchy.",
    ],
  },

  {
    slug: "blueberry-zucchini-bread",
    title: "Blueberry Zucchini Bread",
    short: "Zucchini Bread",
    category: "Baked Goods",
    yield: "One 9×5 loaf · 10 slices",
    time: { prep: "20 min", cook: "60 min", total: "1 hr 20 min + cooling" },
    difficulty: "Easy",
    heroImage: placeholderImg("blueberry-zucchini-bread", "hero"),
    ingredientsImage: placeholderImg("blueberry-zucchini-bread", "ingredients"),
    tested: false,
    ingredients: [
      { amount: "1 1/2 cups", item: "grated zucchini (about 1 medium), squeezed dry" },
      { amount: "1 3/4 cups", item: "all-purpose flour, plus 1 tbsp for the berries" },
      { amount: "1 tsp", item: "baking soda" },
      { amount: "1/2 tsp", item: "baking powder" },
      { amount: "1 tsp", item: "ground cinnamon" },
      { amount: "1/2 tsp", item: "fine salt" },
      { amount: "2", item: "large eggs" },
      { amount: "3/4 cup", item: "granulated sugar" },
      { amount: "1/2 cup", item: "neutral oil" },
      { amount: "1 tsp", item: "vanilla extract" },
      { amount: "1 1/2 cups", item: "fresh blueberries", berries: 148 },
    ],
    steps: [
      "Preheat oven to 350°F (175°C). Grease a 9×5-inch loaf pan and line with a parchment sling.",
      "Grate the zucchini, then wring it out hard in a clean towel. This step decides whether the loaf bakes through or stays gummy — you want the zucchini damp, not wet.",
      "Whisk the 1 3/4 cups flour, baking soda, baking powder, cinnamon, and salt.",
      "In a large bowl, whisk eggs, sugar, oil, and vanilla until smooth. Stir in the squeezed zucchini.",
      "Fold the dry ingredients in until just combined.",
      "Toss the blueberries with 1 tablespoon flour and fold in gently.",
      "Bake 55–60 minutes, until a skewer comes out clean. Tent with foil at 40 minutes if browning early.",
      "Cool 15 minutes in the pan, then turn out and cool completely before slicing.",
    ],
    notes: [
      "You cannot taste the zucchini. It is there for moisture, and it works.",
      "Under-squeezed zucchini is the single reason this loaf ever fails.",
    ],
  },
];

export function getRecipeBySlug(slug: string) {
  return RECIPES.find((r) => r.slug === slug);
}

export function getRecipesByCategory(cat: RecipeCategory) {
  return RECIPES.filter((r) => r.category === cat);
}
